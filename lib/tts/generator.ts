 ...
 * @returns TTS 生成结果
 */
export async function generateTTS(text: string, voice: string = 'af_alloy'): Promise<TTSResult> {
  try {
    if (!text) {
      return {
        success: false,
    ...
    if (!bucketName) {
      return {
        success: false,
        error: 'Bucket name is missing. Set R2_BUCKET_NAME or include it in R2_ENDPOINT.'
      };
    }
