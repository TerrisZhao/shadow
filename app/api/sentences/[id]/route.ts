    ...
    if (audioUrl !== undefined) {
      ...
      if (sentence.userId !== currentUserId) {
        ...
        if (!user.role || !['admin', 'owner'].includes(user.role)) {
          return NextResponse.json({ error: '无权限更新此句子的音频' }, { status: 403 });
        }
      }
...
export async function PUT(
  ...
  try {
    ...
    // 检查权限：只有句子所有者可以更新
    if (sentence.userId !== currentUserId) {
      return NextResponse.json({ error: '您只能编辑自己创建的句子' }, { status: 403 });
    }
  ...
  try {
    ...
    if (sentence.userId !== currentUserId) {
      ...
      if (!isAdmin || !sentence.isShared) {
        return NextResponse.json({ error: '您只能删除自己创建的句子或管理员可删除共享句子' }, { status: 403 });
      }
    }
