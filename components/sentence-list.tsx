...
import { MoreVertical, Heart, HeartOff, Play, Pause, Volume2, Trash2, Edit } from 'lucide-react';
...
export default function SentenceList({ onRefresh, tab = 'shared' }: SentenceListProps) {
  ...
  const [removingItems, setRemovingItems] = useState<Map<number, number>>(new Map()); // sentenceId -> countdown
  ...
  // 获取句子列表
  const fetchSentences = async (page = 1, categoryId = 'all', difficulty = 'all', tabFilter = 'shared') => {
    setLoading(true);
    try {
      ...
      if (response.ok) {
        ...
        setTotalPages(Math.ceil(data.sentences.length / limit) + (data.pagination.hasMore ? 1 : 0));
      }
    } catch (error) {
      ...
      <div className="flex gap-4 items-center">
        ...
        >
          ...
          <SelectItem key="easy" textValue="简单">
            <div className="flex items-center gap-2">
              <Chip size="sm" color="success" variant="flat">简单</Chip>
            </div>
          </SelectItem>
          <SelectItem key="medium" textValue="中等">
            <div className="flex items-center gap-2">
              <Chip size="sm" color="warning" variant="flat">中等</Chip>
            </div>
          </SelectItem>
          <SelectItem key="hard" textValue="困难">
            <div className="flex items-center gap-2">
              <Chip size="sm" color="danger" variant="flat">困难</Chip>
            </div>
          </SelectItem>
        </Select>
      </div>
          ...
          <AnimatePresence mode="popLayout">
            {sentences.map((sentence) => {
              ...
              return (
                ...
                >
                  <Card className={`hover:shadow-md transition-shadow ${isRemoving ? 'opacity-60 border-2 border-danger' : ''}`}>
                    <CardBody className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <Chip
                      size="sm"
                      style={{ backgroundColor: sentence.category.color, color: 'white' }}
                    >
                  ...
                  <Dropdown>
                    ...
                    >
                      {!sentence.audioUrl ? (
                        <DropdownItem 
                          key="generate-audio"
                          onClick={() => generateAudio(sentence.id, sentence.englishText)}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        onPress={() => {
                          // 取消移除倒计时
                          const timer = timersRef.current.get(sentence.id);
                          if (timer) {
                            clearInterval(timer);
